...
## 5-5. 공공데이터·공공누리 사진 채우기 (애드센스 승인에 중요)

지역 페이지에 **실제 주차장 목록**과 **실사 사진**을 넣으면, 17개 지역 페이지가 서로 완전히
달라져 "낮은 가치의 콘텐츠" 판정을 피할 수 있습니다. 둘 다 **data.go.kr 인증키 하나**로 됩니다.

### 1단계 — 인증키 발급 (5분, 무료)

1. https://www.data.go.kr 회원가입 후 로그인
2. **「전국주차장정보표준데이터」** 검색 → 오픈API → **활용신청** (자동 승인)
3. **「한국관광공사_관광사진정보」** 검색 → 오픈API → **활용신청** (자동 승인)
4. 마이페이지 → **인증키 발급현황** → **일반 인증키(Decoding)** 복사

### 2단계 — 데이터 받기

```powershell
$env:DATA_GO_KR_KEY="복사한_인증키"
npm run data      # 지역별 실제 공영주차장 목록 → src/data/parking-lots.json
npm run photos    # 지역별 공공누리 사진   → public/photos/*.webp
npm run build     # 확인
```
...
```bash
git add -A
git commit -m "공공데이터 주차장 목록과 공공누리 사진 반영"
git push
```

> 두 파일이 없으면 주차장 표는 표시되지 않고 사진 자리에는 SVG 일러스트가 쓰입니다.
> 즉 **키가 없어도 사이트는 정상 동작**하며, 나중에 언제든 채워 넣을 수 있습니다.

> **공공누리 제1유형은 출처 표시가 의무**입니다. 사진 아래에 저작권자와 출처가 자동으로
> 표기되므로 그 캡션을 지우면 안 됩니다.
